...
- [Der seriellen Datenbus](der_serielle_monitor.md)
...
Die Kommunikation des Sensors mit dem Mikrokontroller läuft über den [seriellen Datenbus](der_serielle_monitor.md) I2C. Anders als bei einfachen digitalen oder analogen Eingängen, können an den Datenbus mehrere I2C-Geräte (wie z.B. Sensoren oder Displays) parallel geschaltet werden. Jedes Gerät hat dabei eine eindeutige Kennung, damit der Datenbus jedes Einzelne davon zuordnen und separat ansprechen kann.
...
***Hinweis:*** *Benutze dazu das Beispiel aus der HDC100X-Bibliothek.*
