...
## Ziele der Station
In dieser Station wollen wir lernen, wie wir eine Internetseite aufrufen die auf der SD Karte des Arduinos gespeichert ist. Über diese Seite sollen dann einzelne Pins des Arduinos geschaltet werden. Das Projekt ist aufwendiger als die Einstiegsprojekte. Du solltest also schon ein bisschen Erfahrung aus anderen Projekten gesammelt haben bevor du mit dem Webserver beginnst. Die Themen Netzwerkzugriff und HTML sind so umfangreich, dass wir sie im Umfang dieses Sketches nur anschneiden können. Weiterführende Informationen sind im Internet aber leicht zu finden. 

## Materialien
* Ethernetshield

#### Materialien die nicht in der senseBox sind
* LAN Kabel
...
Wir werden nur über das lokale Netzwerk auf den Arduino zugreifen. Es ist oft sehr schwer über das Internet auf ein Schulnetzwerk
 zu zugreifen. Private Router haben eine dynamische IP-Adresse, das heißt euer Internetanbieter ändert eure Adresse täglich. Es gibt Möglichkeiten diese Zugriffe auch über das Internet zu ermöglichen, dazu könnt [hier](http://www.pcwelt.de/ratgeber/Crashkurs__Fernzugriff_und_Portfreigaben-Via_Internet_ins_Heimnetz-8324897.html) weitere Informationen finden. 

## Aufbau
grafik fehlt noch

## Aufgabe 1
Ihr müsste für den Server drei Bibliotheken importieren. 
```
#include <Ethernet.h>
#include <TextFinder.h>
#include <SD.h>
``` 
**Achtung** *Es ist wichtig, dass ihr die Ethernet-Bibliothek von unsere [Website](https://github.com/sensebox/resources/blob/master/libraries/senseBox_Libraries.zip?raw=true) benutzt. Mit der Standartbibliothek vom Arduino funktioniert unser Shield nicht. Die Arduinoumgebung wird die Ethernet-Bibliothek immer 'aktualisieren' wollen. Das dürft ihr nicht machen.*

Anschließend müssen in zwei Variablen die MAC-Adresse und die IP-Adresse des Arduino-Servers fest legen. Zusätzlich muss der Pin 4 als SD-Karten Pin festgelegt werden. Außerdem definieren wir uns ein Objekt 'File' indem die HTML Seite abgelegt wird. 
